...
def main():
    connection.close()
    # estimate buy
    p1 = estimate_buy(connection=connection)
    ...
    if len(p2["positionIds"]) > 0:
        p2 = estimate_sell(connection=connection, positionId=p2["positionIds"][0])
        ...
        if condition_match:
            resp = close_position(connection=connection, positionId=p2["positionId"], spotPriceTick=p2["tick"])
