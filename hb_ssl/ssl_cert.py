...
# basic code copied from hummingbot
...
CONF_DIR_PATH = f'{root_path}/conf'
...
def generate_private_key(password, filepath):
    ...
    if password:
        logging.info(f'generate_private_key: {"".join(["*" for _ in password])} {filepath}')
    else:
        logging.info(f'generate_private_key: {password} {filepath}')
...
def generate_public_key(private_key, filepath):
    ...
    logging.info(f'generate_public_key: {private_key} {filepath}')
...
def generate_csr(private_key, filepath):
    ...
    logging.info(f'generate_csr: {private_key} {filepath}')
...
def sign_csr(csr, ca_public_key, ca_private_key, filepath):
    ...
    logging.info(f'sign_csr: {csr} {ca_public_key} {filepath}')
...
def create_self_sign_certs(pass_phase: str, cert_path: str):
    ...
    logging.info(f'create_self_sign_certs: {pass_phase} {cert_path}')
